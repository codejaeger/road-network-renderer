...
namespace soc{
	Path::Path(){
		...
		std::string vertex_shader_file("./src/Bezier-Curve/vertex-shaders/v_bezier.glsl");
        std::string fragment_shader_file("./src/Bezier-Curve/fragment-shaders/f_bezier.glsl");
	...
	std::vector<glm::vec2> Path::bezier_curve_point(std::vector<glm::vec2> pos, float ratio)
	{
	  ...
	  {
	    temp = glm::vec2((pos[i][0] * (1 - ratio)) + (pos[i+1][0] * ratio), (pos[i][1] * (1 - ratio)) + (pos[i+1][1] * ratio));
	...
	void Path::getPoints(GLFWwindow * window){
		...
		int rt_state = glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT);
    	/* Is true whenever right button is clicked */
	    if (state == GLFW_PRESS)
	    {
	      ...
	      // for (int i = 0; i < beizer_curve_positions.size() ; i++)
	      // {
	      //   std::cout << beizer_curve_positions[i][0] << ", " <<beizer_curve_positions[i][1]<<endl;
	      // }
